...
class XLMRobertaSelfAttention(nn.Module):
    ...
    def _manual_attention(
        self, queries, keys, values, attention_mask=None, head_mask=None
    ):
        attention_scores = (queries @ keys.swapaxes(-1, -2)) * self.scale
        ...
        return attention_probs @ values, attention_probs

    def __call__(
        self, x: mx.array, attention_mask=None, head_mask=None, output_attentions=False
    ):
        queries = self.transpose_for_scores(self.query(x))
        keys = self.transpose_for_scores(self.key(x))
        values = self.transpose_for_scores(self.value(x))

        if output_attentions or head_mask is not None:
            context_layer, attention_probs = self._manual_attention(
                queries, keys, values, attention_mask, head_mask
            )
        else:
...
class Model(nn.Module):
    ...
    def get_extended_attention_mask(self, attention_mask, dtype):
        ...
        negative_value = mx.array(-10000.0, dtype=dtype)
        return (one - extended_attention_mask) * negative_value

    def get_head_mask(self, head_mask, num_hidden_layers):
        if head_mask is None:
            return None

        head_mask = mx.array(head_mask)
        if head_mask.ndim == 1:
            head_mask = head_mask[None, :, None, None]
            head_mask = mx.broadcast_to(
                head_mask, (num_hidden_layers, head_mask.shape[1], 1, 1)
            )
        elif head_mask.ndim == 2:
            head_mask = head_mask[:, :, None, None]
        elif head_mask.ndim != 4:
            raise ValueError(
                "head_mask must have shape [num_heads], "
                "[num_layers, num_heads], or [num_layers, num_heads, 1, 1]"
            )
        return head_mask
    ...
    def __call__(
        ...
        if attention_mask is None:
            attention_mask = (input_ids != self.config.pad_token_id).astype(mx.float32)
        else:
        ...
        extended_attention_mask = self.get_extended_attention_mask(
            attention_mask, dtype=embedding_output.dtype
        )
